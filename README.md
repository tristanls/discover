...
Discover is a distributed master-less node discovery mechanism that enables locating any entity (server, worker, drone, actor) based on node id. 
...
' encoding was picked over 'hex' encoding because it takes up less space when printed or serialized in ASCII over the wire.

#### new Discover(options)

  * `options`:
    * `transport`: _Object_ _(Default: `discover-tcp-transport`)_ An optional initialized and ready to use transport module for sending communications that conforms to the Transport Protocol.
...
Modules implementing the transport mechanism for Discover shall conform to the following protocol. The `transport` is a JavaScript object.
...
#### transport.ping(contact)
  ...
  * `contact`: _Object_ The contact to ping
